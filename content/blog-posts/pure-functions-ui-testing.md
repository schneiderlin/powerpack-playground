...
In our architecture, the UI is produced by **view functions**: they take application state and return a **hiccup** tree (data), not DOM or pixels. For the broader picture of state → IR → output, see [可复用的 UI 代码](./what-is-ui.md). For example:
...
Giving the agent a concrete suggestion (e.g. “maybe try :partial true?”) acts as a **guardrail**: it keeps the agent from guessing or going off track. On why guardrails matter for AI-driven workflows, see [Guardrail 的必要性](./agent-guardrail-necessity.md) and [长时间运行的 Agent：设计原则与理论框架](./long-running-agents.md).
