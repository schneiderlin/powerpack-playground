(ns powerblog.static.software-engineering-page
  (:require
   [clojure.string :as str]
   [datomic.api :as d]))

(defn get-software-engineering-posts [db]
  (->> (d/q '[:find [?e ...]
              :in $ ?tag
              :where
  ...
  [:div {:class "layout-container layout-section-spacing"}
   [:section {:class "section-block"}
    ...
    [:p {:class "text-lg text-gray-600 max-w-2xl"}
     "系统性工程思考类文章，探讨架构设计、工程实践与开发理念。"]]
   [:section {:class "section-block"}
    [:div {:class "section-label"}
     "Software Engineering Articles"]
    [:div {:class "layout-grid-3"}
     (for [post (get-software-engineering-posts db)]
       (blog-post-card post))]]])
